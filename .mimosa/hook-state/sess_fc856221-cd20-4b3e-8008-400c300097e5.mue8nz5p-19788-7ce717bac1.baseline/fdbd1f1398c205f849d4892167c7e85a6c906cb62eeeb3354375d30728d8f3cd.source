...
import type { MatchingSettings as MatchingSettingsType } from './MatchingTypes';

interface MatchingSettingsProps {
  isOpen: boolean;
  settings: MatchingSettingsType;
  onClose: () => void;
  onUpdateSettings: (settings: Partial<MatchingSettingsType>) => void;
}
      ...
      >
        <div className="p-5 border-b border-slate-100 flex items-center justify-between gap-3 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-black text-slate-800 leading-tight">Cấu hình Ghép nối</h3>
              <p className="text-[10px] font-bold text-slate-400 mt-0.5">Thiết lập bài tập ghép nối</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="p-5 space-y-4 overflow-y-auto flex-1 text-[10px]">
          <div className="space-y-3">
            <Checkbox
              checked={localSettings.shuffleAnswers !== false}
              onChange={(checked) => handleUpdateField({ shuffleAnswers: checked })}
              label={<span className="font-black text-slate-600">Trộn cột đáp án bên phải</span>}
            />
            <Checkbox
              checked={localSettings.allowRetries !== false}
              onChange={(checked) => handleUpdateField({ allowRetries: checked })}
              label={<span className="font-black text-slate-600">Cho phép học sinh làm lại bài</span>}
            />
            <Checkbox
              checked={localSettings.showScore !== false}
              onChange={(checked) => handleUpdateField({ showScore: checked })}
              label={<span className="font-black text-slate-600">Hiển thị số cặp ghép đúng</span>}
            />
            <Checkbox
              checked={localSettings.showFeedback !== false}
              onChange={(checked) => handleUpdateField({ showFeedback: checked })}
              label={<span className="font-black text-slate-600">Hiển thị thông báo kết quả làm bài</span>}
            />
            <Checkbox
              checked={!!localSettings.revealAnswers}
              onChange={(checked) => handleUpdateField({ revealAnswers: checked })}
              label={<span className="font-black text-slate-600">Cho phép xem đáp án đúng</span>}
            />
            <Checkbox
              checked={!!localSettings.autoCheck}
              onChange={(checked) => handleUpdateField({ autoCheck: checked })}
              label={<span className="font-black text-slate-600">Tự động kiểm tra sau mỗi đường nối</span>}
            />
          </div>
