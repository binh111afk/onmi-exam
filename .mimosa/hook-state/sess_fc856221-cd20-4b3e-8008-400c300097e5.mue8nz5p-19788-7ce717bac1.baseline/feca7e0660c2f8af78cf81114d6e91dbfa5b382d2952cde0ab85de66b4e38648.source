...
import { questionDocumentToOml } from '../document/adapters/omlQuestionObjectAdapter';
...
const toPipelineSummary = (pipeline: DocumentPipelineResult): BenchmarkPipelineSummary => ({
  ...
  omlDocument: questionDocumentToOml(pipeline.document),
});
...
const HttpDiagnostics = ({ reports }: { reports: OcrDiagnosticReport[] }) => {
  const [tab, setTab] = useState<'summary' | 'http'>('summary');
  const requests = reports.flatMap((report) => report.http.map((http, index) => ({ report, http, response: report.providerResponses[index] ?? null, adapter: report.adapterDiagnostics[index] ?? null })));
  if (requests.length === 0) return null;
  return <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center justify-between gap-3"><div><h2 className="text-lg font-black text-slate-900">HTTP Diagnostics</h2><p className="mt-1 text-sm text-slate-500">Headers và raw response được giữ nguyên trong JSON export.</p></div><div className="flex rounded-lg border border-slate-200 p-1"><button onClick={() => setTab('summary')} className={`rounded-md px-3 py-1.5 text-sm font-bold ${tab === 'summary' ? 'bg-slate-900 text-white' : 'text-slate-600'}`}>Summary</button><button onClick={() => setTab('http')} className={`rounded-md px-3 py-1.5 text-sm font-bold ${tab === 'http' ? 'bg-slate-900 text-white' : 'text-slate-600'}`}>HTTP</button></div></div>{tab === 'summary' ? <div className="mt-4 overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="px-3 py-2">Provider</th><th className="px-3 py-2">HTTP</th><th className="px-3 py-2">Finish</th><th className="px-3 py-2">Candidates / Parts</th><th className="px-3 py-2">Model version</th><th className="px-3 py-2">Response bytes</th></tr></thead><tbody>{requests.map(({ http, response }, index) => <tr key={`${http.provider}-${http.status}-${index}`} className="border-t border-slate-100"><td className="px-3 py-2">{http.provider} · {http.model}</td><td className="px-3 py-2">{http.status} {http.statusText}</td><td className="px-3 py-2">{response?.finishReason ?? '—'}</td><td className="px-3 py-2">{response ? `${response.candidateCount} / ${response.partsCount}` : '—'}</td><td className="px-3 py-2">{response?.modelVersion ?? '—'}</td><td className="px-3 py-2">{http.responseBytes}</td></tr>)}</tbody></table></div> : <div className="mt-4 space-y-3">{requests.map(({ http, response, adapter }, index) => <details key={`${http.provider}-${http.status}-${index}`} className="rounded-xl border border-slate-200 p-4"><summary className="cursor-pointer font-bold text-slate-800">{http.provider} · {http.model} · HTTP {http.status} {http.statusText}</summary><dl className="mt-3 grid gap-3 text-sm md:grid-cols-2"><div><dt className="text-slate-500">Endpoint</dt><dd className="break-all font-medium">{http.endpoint}</dd></div><div><dt className="text-slate-500">Finish / Block reason</dt><dd className="font-medium">{response?.finishReason ?? '—'} / {response?.blockReason ?? '—'}</dd></div><div><dt className="text-slate-500">Tokens / Usage</dt><dd className="font-medium">{JSON.stringify(response?.usageMetadata ?? null)}</dd></div><div><dt className="text-slate-500">Safety / Prompt feedback</dt><dd className="font-medium">{JSON.stringify({ safetyRatings: response?.safetyRatings ?? null, promptFeedback: response?.promptFeedback ?? null })}</dd></div></dl>{adapter?.exception && <pre className="mt-3 max-h-64 overflow-auto rounded-lg bg-red-950 p-3 text-xs text-red-50">{adapter.exception}</pre>}<p className="mt-4 text-xs font-black uppercase tracking-wide text-slate-500">Headers</p><pre className="mt-2 max-h-64 overflow-auto rounded-lg bg-slate-950 p-3 text-xs text-slate-100">{JSON.stringify(http.headers, null, 2)}</pre><p className="mt-4 text-xs font-black uppercase tracking-wide text-slate-500">Raw response</p><pre className="mt-2 max-h-96 overflow-auto rounded-lg bg-slate-950 p-3 text-xs text-slate-100">{http.rawBody}</pre></details>)}</div>}</section>;
};
...
export const DocumentBenchmark = () => {
  ...
  const run = async (sourceFiles: BenchmarkFileSource[]) => {
    ...
    const dispatcher = new DocumentPipelineDispatcher({ benchmarkRunOptions, mode: 'benchmark' });
    for (const source of sourceFiles) {
      ...
      try {
        ...
        setResults((current) => current.map((result) => result.id === source.id ? { ...result, status: summary.documentStatus === 'partial-success' ? 'partial-success' : 'passed', pipeline: summary, finishedAt: new Date().toISOString(), currentStep: summary.documentStatus === 'partial-success' ? 'Hoàn tất một phần' : 'Hoàn tất' } : result));
      } catch (error) {
  ...
  const completed = results.filter((result) => result.status === 'passed' || result.status === 'partial-success' || result.status === 'failed' || result.status === 'skipped');
  ...
  const exportCsv = () => {
    ...
    const rows = ['row_type,file,mode,document_status,total_pages,passed_pages,failed_pages,passed_regions,failed_regions,review_markers,question_count,request_id,page,region_id,rejection_id,rejection_type,rejection_reason,rejection_preview'];
    results.forEach((result) => {
      const metrics = result.pipeline?.metrics;
      rows.push(csvRow(['document', result.relativePath, metrics?.mode ?? 'strict', result.pipeline?.documentStatus ?? 'failed', metrics?.totalPages ?? 0, metrics?.passedPages.join('|') ?? '', metrics?.failedPages.join('|') ?? '', metrics?.passedRegions.join('|') ?? '', metrics?.failedRegions.map((region) => `${region.page}:${region.id}`).join('|') ?? '', resultCounts(result).review, resultCounts(result).question, '', '', '', '', '', '', '']));
      (metrics?.ocrProfiler ?? []).forEach((record) => rows.push(csvRow(['request', result.relativePath, metrics?.mode ?? 'strict', result.pipeline?.documentStatus ?? 'failed', '', '', '', '', '', '', '', record.id, record.page, record.regionId, '', '', '', ''])));
      const reports = [...(result.pipeline?.metrics.ocrDiagnostics ?? []), ...(result.errorDetails?.ocrDiagnostics ?? [])];
      reports.forEach((report) => report.rejectedBlocks.forEach((block) => rows.push(csvRow(['rejection', result.relativePath, metrics?.mode ?? 'strict', result.pipeline?.documentStatus ?? 'failed', '', '', '', '', '', '', '', '', block.page ?? '', '', block.id ?? '', block.type ?? '', block.reason, block.preview]))));
    });
    download('document-ocr-diagnostics.csv', 'text/csv;charset=utf-8', rows.join('\n'));
  };
  const exportMarkdown = () => download('document-benchmark.md', 'text/markdown;charset=utf-8', [
    '# Document Benchmark', '', `Files: ${results.length}`, `Passed: ${results.filter((result) => result.status === 'passed').length}`, `Partial success: ${results.filter((result) => result.status === 'partial-success').length}`, `Failed: ${results.filter((result) => result.status === 'failed').length}`, `Average pipeline: ${formatMilliseconds(summary.averagePipeline)}`,
    '', '## Document status', '| File | Mode | Status | Pages passed / total | Failed pages | Regions passed / failed | Review markers | Questions |', '| --- | --- | --- | --- | --- | --- | ---: | ---: |',
    ...results.map((result) => { const metrics = result.pipeline?.metrics; const counts = resultCounts(result); return `| ${result.relativePath} | ${metrics?.mode ?? 'strict'} | ${result.pipeline?.documentStatus ?? 'failed'} | ${metrics ? `${metrics.passedPages.length}/${metrics.totalPages}` : '0/0'} | ${metrics?.failedPages.join(', ') || '—'} | ${metrics ? `${metrics.passedRegions.length}/${metrics.failedRegions.length}` : '0/0'} | ${counts.review} | ${counts.question} |`; }),
    '', '## Page pipeline status', '| File | Page | Status | OCR | Filter #1 | Merge | Filter #2 | Stopped at | Exception |', '| --- | ---: | --- | ---: | ---: | ---: | ---: | --- | --- |', ...pageStatuses.map((status) => `| ${status.file} | ${status.page ?? '—'} | ${status.status} | ${status.ocrBlocks} | ${status.filter1Blocks} | ${status.mergeBlocks} | ${status.filter2Blocks} | ${status.failedStage ?? '—'} | ${status.exception ?? '—'} |`),
    '', '## OCR Profiler Summary', '', `Total requests: ${ocrProfilerSummary.totalRequests}`, '| Phase | Average | Median | P95 | P99 |', '| --- | ---: | ---: | ---: | ---: |', ...[['Request', ocrProfilerSummary.request], ['Crop', ocrProfilerSummary.crop], ['JPEG encode', ocrProfilerSummary.jpegEncode], ['Base64 encode', ocrProfilerSummary.base64Encode], ['Upload', ocrProfilerSummary.upload], ['Provider inference', ocrProfilerSummary.provider], ['Download', ocrProfilerSummary.download], ['Response parse', ocrProfilerSummary.responseParse]].map(([name, metric]) => `| ${name} | ${formatMilliseconds(metric.average ?? undefined)} | ${formatMilliseconds(metric.median ?? undefined)} | ${formatMilliseconds(metric.p95 ?? undefined)} | ${formatMilliseconds(metric.p99 ?? undefined)} |`),
    '', '## Full OCR Diagnostics JSON', '```json', JSON.stringify(ocrDiagnostics, null, 2), '```',
  ].join('\n'));
    ...
    <main className="min-h-screen bg-slate-50 px-5 py-6 lg:px-8">
      <section className="mx-auto max-w-[1600px] space-y-6">
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          {[
            ['Tổng tệp', results.length], ['Đã xử lý', `${completed.length}/${results.length}`], ['PASS', results.filter((result) => result.status === 'passed').length], ['PARTIAL PASS', results.filter((result) => result.status === 'partial-success').length], ['FAIL', results.filter((result) => result.status === 'failed').length], ['Còn lại', results.filter((result) => result.status === 'pending').length],
        ...
        {pipelineResults.length > 0 && <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-lg font-black text-slate-900">OCR completion</h2><div className="mt-4 grid gap-3 lg:grid-cols-2">{pipelineResults.map((result) => { const metrics = result.pipeline?.metrics; const counts = resultCounts(result); if (!metrics) return null; return <div key={`${result.id}-completion`} className="rounded-xl border border-slate-200 p-4"><p className="font-bold text-slate-900">{result.name}</p><p className="mt-1 text-sm font-semibold text-slate-600">{result.pipeline?.documentStatus === 'partial-success' ? 'PARTIAL PASS' : result.pipeline?.documentStatus === 'pass' ? 'PASS' : 'FAIL'} · {metrics.mode}</p><dl className="mt-3 grid grid-cols-2 gap-3 text-sm"><div><dt className="text-slate-500">Pages passed / total</dt><dd className="font-bold">{metrics.passedPages.length} / {metrics.totalPages}</dd></div><div><dt className="text-slate-500">Failed pages</dt><dd className="font-bold">{metrics.failedPages.join(', ') || '—'}</dd></div><div><dt className="text-slate-500">Regions passed / failed</dt><dd className="font-bold">{metrics.passedRegions.length} / {metrics.failedRegions.length}</dd></div><div><dt className="text-slate-500">Questions / review markers</dt><dd className="font-bold">{counts.question} / {counts.review}</dd></div></dl></div>; })}</div></section>}
        ...
        <HttpDiagnostics reports={ocrDiagnostics} />
