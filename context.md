...
- **Còn lại (chờ user)**: commit toàn bộ; Manage outline screen; Import tài liệu; published status (cần model); AI side panel; handleAiSuggest-empty; dead-code cleanup (handleAddFolder/handleCreateSubLesson/handleCreateDocumentInFolder/handleDepthExceeded + handleSideToolClick + token indigo-150).
