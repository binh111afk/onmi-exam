...
import type { CodeLanguage } from './blocks/code/CodeTypes';
...
export const DocPreviewSimulator: React.FC<DocPreviewSimulatorProps> = ({
  ...
  const {
    isPreviewFullscreenOpen: isFullscreenOpen,
    isPreviewFullscreenClosing: isFullscreenClosing,
    previewZoom,
    isPreviewFitWidth: isFitWidth,
    setIsPreviewFitWidth: setIsFitWidth,
    previewScrollRef,
    fullscreenPreviewScrollRef: fullscreenScrollRef,
    openPreviewFullscreen: openFullscreen,
    closePreviewFullscreen: closeFullscreen,
    applyPreviewZoom: applyZoom,
    stepPreviewZoom: stepZoom,
  } = usePreviewViewport();
      ...
      <div className="space-y-4 text-[11px] leading-relaxed text-slate-900">
        {blocks.map((block, idx) => {
          ...
          if (block.type === 'code') {
            ...
            return (
              <div key={block.id} className="my-2.5 select-text">
  ...
  return (
    <aside className={variant === 'full' ? 'flex-1 bg-[#F8FAFC] flex flex-col overflow-hidden shrink-0 transition-all duration-300' : 'w-[460px] bg-[#F8FAFC] border-r border-slate-100 flex flex-col overflow-hidden shrink-0 transition-all duration-300'}>
