...
import { X, Clock, Workflow, Layers, Columns, Network, Link, TextCursorInput, Move, ListOrdered, Brain } from 'lucide-react';
...
export const OtherBlocksPopup: React.FC<OtherBlocksPopupProps> = ({
  ...
  return (
    <div
      className="fixed inset-0 z-55 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm transition-opacity duration-300"
      onClick={onClose}
    >
      <div
        className="bg-white rounded-3xl p-6 max-w-md w-full mx-4 shadow-2xl border border-slate-100 relative animate-fadeIn"
      ...
      >
        ...
        <div className="flex flex-col gap-3 max-h-[360px] overflow-y-auto pr-1">
          <OtherBlockCard
            onClick={() => onSelectBlock('mindmap')}
            icon={<Brain size={18} />}
            title="Mindmap"
            description="Sơ đồ tư duy dạng nhánh"
          />
