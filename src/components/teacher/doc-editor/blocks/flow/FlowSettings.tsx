        ...
        <div className="p-5 space-y-4">
          <div className="flex items-center justify-between gap-4">
            ...
            <select
              ...
              onChange={(e) => onUpdateSettings({ layout: e.target.value as 'vertical' | 'horizontal' | 'zigzag' })}
          ...
          <div className="flex items-center justify-between gap-4">
            <span className="text-[10px] font-black text-slate-600 uppercase tracking-wider">Kiểu mũi tên</span>
            <select
              value={settings.arrowStyle}
              onChange={(e) => onUpdateSettings({ arrowStyle: e.target.value as 'straight' | 'dashed' | 'curved' })}
              className="bg-white border border-slate-200 focus:border-primary rounded-xl px-3 py-2 text-[10px] font-bold text-slate-800 outline-none focus:ring-1 focus:ring-primary/10 transition cursor-pointer"
            >
