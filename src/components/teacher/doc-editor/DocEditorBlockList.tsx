...
const BlockRowComponent: React.FC<BlockRowProps> = ({
  ...
  return (
    <div style={indentStyle} className={`block-row-item ${block.type === 'table' ? 'w-fit min-w-full' : 'w-full'}`}>
      <BlockWrapper
        block={block}
        idx={idx}
...
export const DocEditorBlockList: React.FC<DocEditorBlockListProps> = ({
  ...
  return (
    <div
      ...
      onKeyDown={(e) => { if (!isPreviewMode) handleKeyDown(e, activeBlockIndex); }}
      ...
      className="flex-1 pl-20 pr-8 py-10 overflow-auto select-text relative"
    >
      <div className="w-full max-w-[820px] space-y-4" onClick={handleScrollWrapperClick}>
