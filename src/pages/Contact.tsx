        ...
        <div className="bg-white rounded-3xl border border-[#E2E8F0]/70 p-6 shadow-sm h-full flex flex-col">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 flex-1 flex flex-col">
            <div>
              ...
              <input type="text" required placeholder="Nhập họ và tên" value={name} onChange={e => setName(e.target.value)} className="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-xs text-[#1E293B] outline-none focus:ring-2 focus:ring-indigo-100 focus:border-primary bg-white transition-all" />
            </div>

            <div>
              <label className="block text-xs font-bold text-[#1E293B] mb-1.5">Địa chỉ Email</label>
              <input type="email" required placeholder="Nhập email của bạn" value={email} onChange={e => setEmail(e.target.value)} className="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-xs text-[#1E293B] outline-none focus:ring-2 focus:ring-indigo-100 focus:border-primary bg-white transition-all" />
            </div>
            ...
            <div className="p-3 border border-dashed border-indigo-100 rounded-xl bg-slate-50/30 text-center cursor-pointer hover:bg-indigo-50/20 transition-colors shrink-0">
              <svg viewBox="0 0 24 24" className="w-4 h-4 text-primary mx-auto mb-1" fill="none" stroke="currentColor" strokeWidth="2">
                <path d="M21.44 11.05l-9.19 9.19a6 6 0 01-8.49-8.49l9.19-9.19a4 4 0 015.66 5.66l-9.2 9.19a2 2 0 01-2.83-2.83l8.49-8.48" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
              <p className="text-[11px] font-bold text-primary">Đính kèm tệp (nếu có)</p>
              <p className="text-[9px] text-slate-400 mt-0.5">Hỗ trợ PDF, PNG, JPG (tối đa 10MB)</p>
            </div>

            <button type="submit" className="w-full py-3 bg-primary hover:bg-[#4F46E5] text-white font-black text-xs rounded-xl flex items-center justify-center gap-1.5 transition-colors cursor-pointer shadow-md shadow-indigo-100 shrink-0">
        ...
        <div className="flex flex-col gap-6 h-full">
          <div className="bg-white rounded-3xl border border-[#E2E8F0]/70 p-6 flex flex-col items-center justify-center text-center shadow-sm flex-1">
            <svg viewBox="0 0 260 200" className="w-full max-w-[180px] mb-4" fill="none">
              ...
              <rect x="56" y="100" width="20" height="30" rx="10" fill="#6C5DD3" />
              <rect x="154" y="100" width="20" height="30" rx="10" fill="#6C5DD3" />
              <path d="M76 130 Q62 144 67 158 L84 158" stroke="#4338CA" strokeWidth="3.5" strokeLinecap="round" />
              <rect x="82" y="153" width="22" height="10" rx="5" fill="#6C5DD3" />
            </svg>
            ...
            <div className="w-full space-y-3.5 text-left">
              {[ { icon: <Clock size={13} />, title: 'Phản hồi nhanh chóng', desc: 'Cam kết trong vòng 24 giờ' }, { icon: <MessageSquare size={13} />, title: 'Hỗ trợ tận tâm', desc: 'Giải đáp mọi thắc mắc' } ].map((f, i) => (
                <div key={i} className="flex gap-2.5"><div className="w-7 h-7 rounded-full bg-indigo-50 flex items-center justify-center text-primary shrink-0 shadow-sm">{f.icon}</div><div><h4 className="text-[11px] font-black text-[#1E293B] leading-none mb-0.5">{f.title}</h4><p className="text-[9px] text-slate-400 font-medium leading-relaxed">{f.desc}</p></div></div>
              ))}
            </div>
          </div>
          <div className="bg-white rounded-3xl border border-[#E2E8F0]/70 p-5 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-2.5">
              {categories.slice(0, 4).map((cat, i) => {
                ...
                return (
                  <button key={i} type="button" onClick={() => setRequestType(reqTypes[i])} className={`p-3 border rounded-2xl flex flex-col items-center text-center transition-all cursor-pointer bg-slate-50/10 shadow-sm ${isSelected ? 'border-primary bg-indigo-50/30 ring-2 ring-indigo-100' : 'border-slate-100 hover:border-primary/55 hover:bg-slate-50/40'}`}>
          ...
          <div className="bg-white rounded-3xl border border-[#E2E8F0]/70 p-5 shadow-sm space-y-4">
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={i} className="flex gap-3 items-start"><div className="w-8 h-8 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-500 shrink-0 shadow-sm">{item.icon}</div><div className="min-w-0"><h4 className="text-[11px] font-bold text-slate-400 uppercase tracking-wider leading-none mb-1">{item.title}</h4><p className={`text-xs font-bold leading-normal truncate ${item.highlight ? 'text-primary' : 'text-[#1E293B]'}`}>{item.info}</p>{item.label && <span className="text-[9px] font-bold text-slate-400 block mt-0.5">{item.label}</span>}</div></div>
              ))}
            </div>
          </div>
