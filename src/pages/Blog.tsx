...
const mockPopularPosts = [
  ...
  {
    id: 'pop-3',
    title: '5 sai lầm khi làm đề thi trắc nghiệm khiến bạn mất điểm đáng tiếc',
    views: '1.8K lượt đọc',
    thumbnailType: 'checklist' as const,
    thumbnailBg: 'bg-[#D1FAE5]',
  }
];
...
const MiniThumbnail: React.FC<{ type: 'calendar' | 'brain' | 'checklist'; bg: string }> = ({ type, bg }) => (
  <div className={`w-16 h-16 rounded-lg shrink-0 ${bg} flex items-center justify-center overflow-hidden`}>
    {type === 'calendar' && (
      <svg viewBox="0 0 48 48" className="w-10 h-10" fill="none" stroke="#6366F1" strokeWidth="2">
...
const CardThumbnail: React.FC<{ type: 'calendar' | 'brain' | 'checklist'; bg: string }> = ({ type, bg }) => (
  <div className={`w-full h-full ${bg} flex items-center justify-center`}>
    {type === 'calendar' && (
      <svg viewBox="0 0 120 90" className="w-24 h-24" fill="none">
    ...
    {type === 'brain' && (
      <svg viewBox="0 0 120 90" className="w-28 h-28" fill="none">
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 antialiased" style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <main className="flex-1 flex flex-col gap-6">
          ...
          {filteredPosts.length === 0 ? (
            <div className="bg-white rounded-2xl p-12 text-center border border-[#E2E8F0]/50" style={{ boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05), 0 2px 4px -1px rgba(0,0,0,0.03)' }}>
              <p className="text-[#64748B] text-sm">Không tìm thấy bài viết trong chuyên mục này.</p>
            </div>
          ) : (
            filteredPosts.map(post => {
              ...
              return (
                <article
                  key={post.id}
                  className="bg-white rounded-2xl p-4 flex flex-col sm:flex-row gap-6 border border-[#E2E8F0]/50 cursor-pointer hover:shadow-md transition-shadow"
                  style={{ boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05), 0 2px 4px -1px rgba(0,0,0,0.03)' }}
                  onClick={() => setSelectedPostId(post.id)}
                >
                  {/* Thumbnail */}
                  <div className={`sm:w-1/3 shrink-0 rounded-xl overflow-hidden ${post.thumbnailBg} min-h-[160px]`}>
                  ...
                  <div className="flex-1 flex flex-col justify-between py-1">
                    <div>
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-3 text-xs">
                          <span className={`px-2.5 py-1 rounded-md font-semibold tracking-wide ${tagStyles[post.category] || 'bg-[#D1FAE5] text-[#047857]'}`}>
                        ...
                        >
                          <svg viewBox="0 0 24 24" className="w-4 h-4" fill={isBookmarked ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2">
                      ...
                      {/* Title */}
                      <h2 className="text-xl font-bold mb-2 text-[#1E293B] hover:text-[#6366F1] transition-colors leading-snug">
                        {post.title}
                      </h2>
                    ...
                    <div className="flex items-center justify-between mt-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex -space-x-2">
                          <div className="w-8 h-8 rounded-full border-2 border-white bg-indigo-500 text-white text-[10px] font-bold flex items-center justify-center">A</div>
                          <div className="w-8 h-8 rounded-full border-2 border-white bg-pink-500 text-white text-[10px] font-bold flex items-center justify-center">B</div>
                          <div className="w-8 h-8 rounded-full border-2 border-white bg-amber-500 text-white text-[10px] font-bold flex items-center justify-center">C</div>
                        </div>
                        <span className="text-sm font-medium text-[#64748B]">{post.views}</span>
                      </div>
                      <span className="text-[#6366F1] font-medium text-sm flex items-center gap-1 hover:underline">
                        Đọc chi tiết
          ...
          {filteredPosts.length > 0 && (
            <div className="flex justify-center items-center gap-2 mt-6">
              <button
                disabled
                className="w-10 h-10 flex items-center justify-center rounded-xl bg-white border border-[#E2E8F0] text-gray-400 hover:bg-gray-50 transition-colors disabled:opacity-50 cursor-not-allowed"
              >
              ...
              <button className="w-10 h-10 flex items-center justify-center rounded-xl bg-[#6366F1] text-white font-medium shadow-md shadow-indigo-200 cursor-pointer">1</button>
              <button className="w-10 h-10 flex items-center justify-center rounded-xl bg-white border border-[#E2E8F0] text-[#1E293B] font-medium hover:bg-gray-50 transition-colors cursor-pointer">2</button>
              <button className="w-10 h-10 flex items-center justify-center rounded-xl bg-white border border-[#E2E8F0] text-[#1E293B] font-medium hover:bg-gray-50 transition-colors cursor-pointer">3</button>
              <span className="w-10 h-10 flex items-center justify-center text-[#64748B]">...</span>
              <button className="w-10 h-10 flex items-center justify-center rounded-xl bg-white border border-[#E2E8F0] text-[#1E293B] font-medium hover:bg-gray-50 transition-colors cursor-pointer">12</button>
              <button className="w-10 h-10 flex items-center justify-center rounded-xl bg-white border border-[#E2E8F0] text-[#1E293B] hover:bg-gray-50 transition-colors cursor-pointer">
                <svg viewBox="0 0 16 16" className="w-3.5 h-3.5" fill="currentColor">
                  <path d="M1 8a.5.5 0 01.5-.5h11.793l-3.147-3.146a.5.5 0 01.708-.708l4 4a.5.5 0 010 .708l-4 4a.5.5 0 01-.708-.708L13.293 8.5H1.5A.5.5 0 011 8z" />
                </svg>
              </button>
        ...
        <aside className="w-full lg:w-[340px] shrink-0 flex flex-col gap-6">
          ...
          {/* Trending Topics */}
          <div
            className="bg-white rounded-2xl p-6 border border-[#E2E8F0]/50"
            style={{ boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05), 0 2px 4px -1px rgba(0,0,0,0.03)' }}
          >
            <div className="flex items-center justify-between mb-5">
              <h3 className="font-bold text-lg text-[#1E293B]">Chủ đề nổi bật</h3>
              <button
            ...
            <ul className="space-y-4">
              {mockTopics.map((topic, index) => (
                <li key={index}>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${topic.color} ${topic.hoverColor} text-sm`}>
                        {topic.icon}
                      </div>
                      <span className="font-medium text-[#1E293B] group-hover:text-[#6366F1] transition-colors text-sm">
                        # {topic.name}
                      </span>
                    </div>
                    <span className="text-xs text-[#64748B] bg-gray-50 px-2 py-1 rounded">
          ...
          >
            ...
            <h3 className="font-bold text-lg mb-2 relative z-10">Đăng ký nhận bản tin</h3>
            <p className="text-sm text-indigo-100 mb-5 leading-relaxed relative z-10">
              Nhận ngay những bài viết mới nhất<br />từ Omni Exam mỗi tuần.
            </p>

            {isSubscribed ? (
              <div className="z-10 bg-white/10 p-4 rounded-xl border border-white/20 text-center space-y-2 relative">
                <div className="w-8 h-8 rounded-full bg-emerald-500/30 text-emerald-300 flex items-center justify-center mx-auto">
              ...
              <form onSubmit={handleSubscribe} className="relative z-10 flex flex-col gap-3">
                <input
                  ...
                  required
                  className="w-full px-4 py-3 rounded-xl border-none text-gray-800 placeholder-gray-400 focus:ring-2 focus:ring-white/50 text-sm outline-none"
                />
                <button
                  type="submit"
                  className="w-full py-3 rounded-xl bg-indigo-700/50 hover:bg-indigo-700/70 text-white font-medium text-sm transition-colors flex justify-center items-center gap-2 border border-white/10 cursor-pointer"
                >
                  Đăng ký ngay
                  <svg viewBox="0 0 16 16" className="w-3 h-3" fill="currentColor">
                    <path d="M1 8a.5.5 0 01.5-.5h11.793l-3.147-3.146a.5.5 0 01.708-.708l4 4a.5.5 0 010 .708l-4 4a.5.5 0 01-.708-.708L13.293 8.5H1.5A.5.5 0 011 8z" />
          ...
          <div
            ...
            style={{ boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05), 0 2px 4px -1px rgba(0,0,0,0.03)' }}
          >
            <h3 className="font-bold text-lg mb-5 text-[#1E293B]">Bài viết được yêu thích</h3>
              ...
              {mockPopularPosts.map((item) => (
                ...
                >
                  ...
                  <div className="flex flex-col justify-center">
                    <h4 className="text-sm font-semibold text-[#1E293B] leading-snug group-hover:text-[#6366F1] transition-colors mb-1 line-clamp-2">
                      {item.title}
                    </h4>
                    <span className="text-xs text-[#64748B]">{item.views}</span>
                  </div>
                </button>
              ))}
            </div>
